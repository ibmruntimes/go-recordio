package main

import (
	"fmt"
	"unicode"

	"github.com/ibmruntimes/go-recordio/v2/utils"
)

func printData(desc string, t [256]byte) {
	toChar := func(b byte) byte {
		if 0 != (b & 0x80) {
			return '.'
		}
		if unicode.IsPrint(rune(b)) {
			return b
		}
		return '.'
	}
	var charstr [16]byte
	fmt.Printf("%s\n", desc)
	fmt.Printf("%02x", t[0])
	i := 0
	charstr[0] = toChar(t[0])
	for i = 1; i < 256; i++ {
		if i%16 == 0 {
			fmt.Printf(" %s\n", string(charstr[0:15]))
		} else {
			fmt.Printf(" ")
		}
		charstr[i&0x0f] = toChar(t[i])
		fmt.Printf("%02x", t[i])
	}
	fmt.Printf(" %s\n", string(charstr[0:15]))
}

var conv819_037 = [256]byte  {
        , }
var conv037_819 = [256]byte  {
        , }

func _etoA(record []byte) {
	sz := len(record)
	for i:=0; i<sz; i++ {
		record[i] = conv037_819[record[i]]
	}
}

func _atoE(record []byte) {
	sz := len(record)
	for i:=0; i<sz; i++ {
		record[i] = conv819_037[record[i]]
	}
}

func main() {
	var t [256]byte
	for i := 0; i < 256; i++ {
		t[i] = byte(i)
	}

	printData("original", t)
	utils.EtoA(t[:])
	printData("converted from Ebcdic to Ascii, ccsid 1047 -> 819", t)
	utils.AtoE(t[:])
	printData("Return to original", t)

	// replace original 1047-819 conversion
	utils.AtoE = _atoE
	utils.EtoA = _etoA
	printData("original", t)
	utils.EtoA(t[:])
	printData("converted from Ebcdic to Ascii, ccsid 037 -> 819", t)
	utils.AtoE(t[:])
	printData("Return to original", t)
}
